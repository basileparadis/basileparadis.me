export function singlePageFeatures() {
  const menuBarHeight = document.getElementsByClassName('main-nav')[0]
    .offsetHeight
  // activate if visible
  ...
  function activateIfVisible() {
    for (let b = true, i = 0; i < navItems.length; i++) {
      if (b && isVisible(navSections[i])) {
        document
          .getElementsByClassName('paradis')[0]
          .classList.add(navItems[i].dataset.target)
        navItems[i].classList.add('is-active')
        b = false
      } else {
        document
          .getElementsByClassName('paradis')[0]
          .classList.remove(navItems[i].dataset.target)
